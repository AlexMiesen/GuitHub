class BookingsController < ApplicationController
  ...
  def create
    ...
    startdate = Date.parse("#{unlocked[:booking]["startdate(1i)"]}-#{unlocked[:booking]["startdate(2i)"]}-#{unlocked[:booking]["startdate(3i)"]}")
    enddate = Date.parse("#{unlocked[:booking]["enddate(1i)"]}-#{unlocked[:booking]["enddate(2i)"]}-#{unlocked[:booking]["enddate(3i)"]}")
    @booking.no_days = enddate.ld - startdate.ld
